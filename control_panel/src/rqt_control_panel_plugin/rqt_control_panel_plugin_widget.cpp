  ...
  nodehandle_(nodehandle)
{
  ...
  ui->Controllers->setTabText(7, "static walk controller");
  ...
  for(auto& controller:list_controllers.response.controller)
    {
      ...
      if(controller.name == "single_leg_controller" && controller.state =="running")
       ...
       if(controller.name == "configure_change_controller" && controller.state =="running")
        ...
        if(controller.name == "static_walk_controller" && controller.state =="running")
        {
            current_controller = "static_walk_controller";
            break;
        }
  ...
  if(tab_name == "Balance")
    {
      ...
      if(controller_switch.response.ok && control_method.response.result)
        ...
        if(controller_switch.response.ok && control_method.response.result)
        {
            ...
            e_stop_msg.data = false;
            eStopPublisher_.publish(e_stop_msg);
            controller_switch.request.start_controllers.push_back("static_walk_controller");
            controller_switch.request.stop_controllers.push_back(current_controller.c_str());
            controller_switch.request.strictness = controller_switch.request.STRICT;
            switchControllerClient_.call(controller_switch.request, controller_switch.response);
...
void rqt_control_panel_plugin_widget::displayOutputInfos(const std::string &color,const QString &context)
{
  if(color == "red")
    {
...
void rqt_control_panel_plugin_widget::on_anti_X_clicked()
{
