...
bool robot_state_callback(free_gait_msgs::optimize::Request&  req,
     ...
     */
        ...
        std::cout << " the estimate base force is " << std::endl;
        std::cout << base_force.transpose() << std::endl;

        bool use_force_estimate;
        use_force_estimate = req.use_force_estimate.data;
        if(use_force_estimate)
        {
            gl[0] = base_force(0,0); gu[0] = gl[0];
            gl[1] = base_force(1,0); gu[1] = gl[1];
            gl[2] = -base_force(2,0); gu[2] = gl[2];//force in the z direction;
            gl[3] = base_force(3,0); gu[3] = gl[3];//roll
            gl[4] = base_force(4,0); gu[4] = gl[4];//yaw
            gl[5] = base_force(5,0); gu[5] = gl[5];//pitch
            std::cout << " use the estimate data" << std::endl;

        }else {
            base_force(0,0) = req.desired_force.wrench.force.x;
            base_force(1,0) = req.desired_force.wrench.force.y;
            base_force(2,0) = req.desired_force.wrench.force.z;
            base_force(3,0) = req.desired_force.wrench.torque.x;
            base_force(4,0) = req.desired_force.wrench.torque.y;
            base_force(5,0) = req.desired_force.wrench.torque.z;
            ...
            std::cout << " No use the estimate data" << std::endl;
        }
        ...
        std::cout << " in the x direction the constraint value is " << xu[25] << " " << xl[25] << std::endl;
        ...
        fg_eval.SetBaseforce(base_force);
        ...
        std::cout << "rf footposition is ";
        foot_position = quadkin.GetFootPositionInWorldframe(free_gait::LimbEnum::RF_LEG);// base frame is the world frame;
        quadkin.CppadPositionPrintf(foot_position);
        std::cout << "rh footposition is ";
        foot_position = quadkin.GetFootPositionInWorldframe(free_gait::LimbEnum::RH_LEG);// base frame is the world frame;
        quadkin.CppadPositionPrintf(foot_position);
        std::cout << "lh footposition is ";
        foot_position = quadkin.GetFootPositionInWorldframe(free_gait::LimbEnum::LH_LEG);// base frame is the world frame;
        quadkin.CppadPositionPrintf(foot_position);
