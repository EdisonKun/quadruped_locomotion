...
void generate_lf_motion(quadruped_model::JointPositionsLimb& lf_start_position, std::vector<JointPositionsLimb>& lf_joint_positions, std::vector<valuetype>& lf_foot_position)
{
    ...
    Pose fd_lf_pose;
    fd_lf_pose.getPosition() << 0.7, 0.18, 0.4;
    QK.InverseKinematicsSolve(fd_lf_pose.getPosition(), LimbEnum::LF_LEG, lf_joint, lf_joint, "IN_RIGHT");
    std::cout << "lf_joint is " << lf_joint << std::endl;//0.285, 3.086, 1.282;
...
//    lf_joint << 0, 1.1, 2.2;
    ...
    lf_leg_position[1].push_back(valuetype(fd_lf_pose.getPosition().x(), fd_lf_pose.getPosition().y() - 0.06, fd_lf_pose.getPosition().z()));
    ...
    lf_leg_position[2].push_back(valuetype(fd_lf_pose.getPosition().x(), fd_lf_pose.getPosition().y() - 0.06, fd_lf_pose.getPosition().z()));

    times_of_lf[2].push_back(period_t);
    lf_leg_position[2].push_back(valuetype(fd_lf_pose.getPosition().x() + 0.1, fd_lf_pose.getPosition().y() + 0.06, fd_lf_pose.getPosition().z() + 0.05));
...
void generate_rf_motion_1(quadruped_model::JointPositionsLimb& rf_start_position, std::vector<JointPositionsLimb>& rf_joint_positions, std::vector<valuetype>& rf_foot_position)
{
    ...
    JointPositionsLimb rf_joint;
    Pose fd_rf_pose;
    fd_rf_pose.getPosition() << 0.7, -0.18, 0.45;
    QK.InverseKinematicsSolve(fd_rf_pose.getPosition(), LimbEnum::RF_LEG, rf_joint, rf_joint, "IN_RIGHT");
    std::cout << "rf_joint is " << rf_joint << std::endl;//0.285, 3.086, 1.282;


    j = 0;
    times_of_rf[j].push_back(0);
    rf_leg_position[j].push_back(valuetype(rf_start_position.x(), rf_start_position.y(), rf_start_position.z()));//Joint angles

    times_of_rf[j].push_back(2 * period_t);
    rf_leg_position[j].push_back(valuetype(rf_joint.x(), -rf_joint.y(), -rf_joint.z()));//

    rf_leg_trajectory[j].fitCurve(times_of_rf[j], rf_leg_position[j]);

    std::cout << "lf_joint is " << rf_joint << std::endl;
    rf_joint.z() = -rf_joint.z();
    rf_joint.y() = -rf_joint.y();
    //    lf_joint << 0, 1.1, 2.2;
    QK.FowardKinematicsSolve(rf_joint, LimbEnum::RF_LEG, fd_rf_pose);
    std::cout << fd_rf_pose.getPosition() << std::endl;


    j = 1;
    times_of_rf[j].push_back(0);
    rf_leg_position[j].push_back(valuetype(fd_rf_pose.getPosition().x(), fd_rf_pose.getPosition().x(), fd_rf_pose.getPosition().x()));//Joint angles

    times_of_rf[j].push_back(period_t);
    rf_leg_position[j].push_back(valuetype(fd_rf_pose.getPosition().x(), fd_rf_pose.getPosition().y() + 0.06, fd_rf_pose.getPosition().z()));//

    rf_leg_trajectory[j].fitCurve(times_of_rf[j], rf_leg_position[j]);

    j = 2;
    times_of_rf[j].push_back(0);
    rf_leg_position[j].push_back(valuetype(fd_rf_pose.getPosition().x(), fd_rf_pose.getPosition().y() + 0.06, fd_rf_pose.getPosition().z()));//Joint angles

    times_of_rf[j].push_back(period_t);
    rf_leg_position[j].push_back(valuetype(fd_rf_pose.getPosition().x(), fd_rf_pose.getPosition().y() + 0.06, fd_rf_pose.getPosition().z() + 0.05));//
    ...
    while(dt <= 4 * period_t )
    {
        ...
        if(dt <= 2 * period_t)
        {
            ...
            rf_leg_joint << rf_leg_pose.getPosition().x(), rf_leg_pose.getPosition().y(), rf_leg_pose.getPosition().z();
        }else {
            rf_leg_pose.getPosition().x() = rf_leg_joint.x();
            rf_leg_pose.getPosition().y() = rf_leg_joint.y();
            rf_leg_pose.getPosition().z() = rf_leg_joint.z();

            QK.InverseKinematicsSolve(rf_leg_pose.getPosition(),LimbEnum::RF_LEG, joints, joints, "OUT_RIGHT");
            joints.z() = -joints.z();
            joints.y() = -joints.y();
        }
...
int main(int argc, char **argv)
{
    ...
    for(unsigned int i = 0; i < lf_joint_positions.size(); i++)
    {
        ...
        joint_positions(3) = lf_joint_positions[i](0);
        joint_positions(4) = -lf_joint_positions[i](1);
        joint_positions(5) = -lf_joint_positions[i](2);
    ...
    quadruped_model::JointPositionsLimb lf_joint;
    lf_joint << 0.424, 3.07464, 1.33097;
    Pose LF_POSE;
