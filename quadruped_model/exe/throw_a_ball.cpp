...
void generate_lf_motion(quadruped_model::JointPositionsLimb& lf_start_position, std::vector<JointPositionsLimb>& lf_joint_positions, std::vector<valuetype>& lf_foot_position)
{
    ...
    Pose fd_lf_pose;
    QK.FowardKinematicsSolve(lf_start_position, LimbEnum::LF_LEG, fd_lf_pose);
    std::cout << "fd_lf_pose is " << fd_lf_pose.getPosition() << std::endl;
    ...
    lf_leg_position[0].push_back(valuetype(fd_lf_pose.getPosition().x(), fd_lf_pose.getPosition().y() - 0.06, fd_lf_pose.getPosition().z()));

    times_of_lf[0].push_back(period_t);
    lf_leg_position[0].push_back(valuetype(fd_lf_pose.getPosition().x() + 0.01, fd_lf_pose.getPosition().y() + 0.01, fd_lf_pose.getPosition().z() + 0.01));
...
void generate_rh_motion_1(quadruped_model::JointPositionsLimb& rh_start_position, std::vector<JointPositionsLimb>& rh_joint_positions, std::vector<valuetype>& rh_foot_position)
{
    ...
    times_of_rh[0].push_back(period_t);
    rh_leg_position[0].push_back(valuetype(rh_start_position.x(), rh_start_position.y(), rh_start_position.z()));
...
int main(int argc, char **argv)
{
    ...
    readfile.open("/home/kun/catkin_ws_dependency/catch_a_ball.txt");
    ...
    SaveAsFile_no_footheight("throw_a_ball.txt",joint_positions_total);
    SaveAsFile("throw_a_ball_read.txt",joint_positions_total, lf_foot_height, rf_foot_height, rh_foot_height, lh_foot_height);
